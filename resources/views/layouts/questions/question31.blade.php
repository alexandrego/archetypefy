        ...
        <div id="formularioQuestions" class="formularioQuestions">
            <form action="/question31" method="POST" onsubmit="avancando()">
                ...
                <div class="message roboto-thin messageStartTest">
                    ...
                    <div class="respostas">
                        @if(session('answer31')=='a')
                            <div class="questions checked">
                                <input type="radio" name="question31" id="a" value="a" checked /><label for="a">Inovar e trazer novas ideias</label>
                            </div>
                        @else
                            <div class="questions">
                                <input type="radio" name="question31" id="a" value="a" required/><label for="a">Inovar e trazer novas ideias</label>
                            </div>
                        @endif

                        @if(session('answer31')=='b')
                            <div class="questions checked">
                                <input type="radio" name="question31" id="b" value="b" checked /><label for="b">Ajudar os outros a realizarem suas tarefas</label>
                            </div>
                        @else
                            <div class="questions">
                                <input type="radio" name="question31" id="b" value="b" required/><label for="b">Ajudar os outros a realizarem suas tarefas</label>
                            </div>
                        @endif

                        @if(session('answer31')=='c')
                            <div class="questions checked">
                        ...
                        @else
                            <div class="questions">
                                <input type="radio" name="question31" id="c" value="c" required/><label for="c">Liderar e garantir que tudo seja feito corretamente</label>
                            </div>
                        @endif

                        @if(session('answer31')=='d')
                            <div class="questions checked">
                                <input type="radio" name="question31" id="d" value="d" checked /><label for="d">Explorar diferentes possibilidades</label>
                            </div>
                        @else
                            <div class="questions">
                                <input type="radio" name="question31" id="d" value="d" required/><label for="d">Explorar diferentes possibilidades</label>
                            </div>
                        @endif
                    </div>
                </div>
